...
传统核保要跨多个系统查资料，人工成本高且依赖经验。本项目让 Agent 做资料聚合、规则检查和建议整理，但模型不拥有最终放行权，确定性规则是决策底线。

展示 README 的架构和两个虚构保单：`P-1001` 高风险仓库、`P-2001` 低风险办公室。
...
指出默认无需外部模型和 API Key。展示知识检索结果中的文档类型、片段、分数，说明流程是解析、切分、向量化、入库、Top-K 检索。

### 第 2–3 分钟：展示业务工具与 MCP

展示 `GET_POLICY` REST 调试结果和 `ToolCallTrace`。再说明同一份业务实现被六个 `@McpTool` 复用，通过 `/mcp` 提供 Streamable HTTP 工具发现，不是用普通 REST 冒充 MCP。

### 第 3–4 分钟：展示完整核保结果

调用 `P-1001` 评估，重点看：

- `MANUAL_REVIEW / HIGH / 70`；
- 命中红色暴雨、重复出险、高保额、整改未完成；
- 4 条 RAG 证据；
- 6 条工具轨迹和 7 条步骤轨迹；
- Mock 模型生成的中文摘要与后续动作。
...
说明模型适配器支持切换、连接/读取超时、429/5xx/I/O 重试、4xx 不重试、默认不静默降级、密钥脱敏。最后说明内存仓库、Hash Embedding、虚构工具都位于接口后，可以替换为 Redis、PostgreSQL/PGVector、企业模型和内部 API。
